...
public class PostController extends BaseController {
    ...
    public String detail(@PathVariable(name = "id") Long id) {
        ...
        //当用户查询大于分页数的数据时返回第一页的数据
        if (results.getCurrent() > 1 && results.getRecords().isEmpty()) {
            results = commentService.paging(new Page(PAGE_CURRENT, PAGE_SIZE), vo.getId(), null, "created");
        }
    ...
    */
    ...
    public Result reply(Long jid, String content, Long commentParentId) {
        ...
        if (content.startsWith("@")) {
            ...
            //判断是否存在被@的用户，和传来父评论id
            if (user != null && commentParentId != null) {

                Comment commentParent = commentService.getById(commentParentId);
                //判断传来的父评论id的作者和被@的用户是否是同一个人
                if (commentParent.getUserId().longValue() == user.getId().longValue()) {
                    //更新评论的父评论id
                    comment.setParentId(commentParentId);
                    commentService.updateById(comment);
                    // 评论者回复自己的评论，不需要通知
                    if (user.getId().longValue() != comment.getUserId().longValue()) {
    ...
    public Result commentDelete(Long id) {
        ...
        if (comment.getUserId().longValue() != getProfileId().longValue()) {
            ...
            //判断是不是贴主
            if (post.getUserId().longValue() != getProfileId().longValue()) {
